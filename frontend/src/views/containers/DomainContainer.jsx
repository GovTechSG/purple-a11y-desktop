...
const crawler = (baseUrl, newScan, props) => {
  axios.post(baseUrl + newScan.call, newScan)
    .then((res) => {
    ...
    if (validateForm(error)) {
      ...
      if (CURRENT_ENV === 'local') {
        ...
        if (res === 1) {
          ...
          crawler(CURRENT_BASE_URL, newScan, props)
        } else {
        ...
        axios.post(CURRENT_BASE_URL + '/api/check', checkDomain).then((res) => {
          const randomToken = generateRandomToken()
          console.log("url checked and valid");
      ...
      <div className="input-fields">
        <div className="select-input">
          <Select
            ...
            placeholder={state.scanMethod === "Website" ? 'https://www.hive.gov.sg' : 'https://www.hive.gov.sg/sitemap.xml'}
            handleChange={e => setState({ ...state, scanMethod: e.target.value })}
          />
          < Input
            type={'text'}
            name={'domain'}
...
export default withRouter(DomainContainer);
