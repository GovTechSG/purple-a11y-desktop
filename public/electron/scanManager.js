...
const startScan = async (scanDetails, scanEvent) => {
  ...
  const response = await new Promise(async (resolve) => {
    ...
    scan.stdout.on("data", async (data) => {
      ...
      if (
        scanDetails.scanType === "custom" &&
        data.includes("generatedScript")
      ) {
...
const getReportPath = (scanId) => {
  ...
  if (scanHistory[scanId]) {
    return path.join(resultsFolderPath, "reports", "report.html");
  }
  return null;
};
...
const init = (scanEvent) => {
  ...
  ipcMain.handle(
    "startReplay",
    async (_event, generatedScript, scanDetails, isReplay) => {
      return await startReplay(
        generatedScript,
        scanDetails,
        scanEvent,
        isReplay
